...
def no_ref_evaluate(pred, pan, hs):
    ...
    c_D_s = D_s(pred, hs, pan, scale=4)
    c_qnr = qnr(pred, hs, pan, scale=4)
    return [c_D_lambda, c_D_s, c_qnr]


# def qnr2(D_lambda_idx, img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=32, p=1, q=1, alpha=1, beta=1):
#     """QNR - No reference IQA"""
...
# def no_ref_evaluate2(c_D_lambda, pred, pan, hs):
#     if pan.ndim == 3 and pan.shape[-1] == 3:
#         pan = pan[:, :, 0]
#     elif pan.ndim == 3 and pan.shape[-1] == 1:
#         pan = pan[:, :, 0]

#     # no reference metrics
#     c_D_s = D_s(pred, hs, np.expand_dims(pan, -1), scale=4)
#     c_qnr = qnr2(c_D_lambda, pred, hs, np.expand_dims(pan, -1), scale=4)
#     return [c_D_lambda, c_D_s, c_qnr]



def SAM(ref, test, eps=1e-8, in_degrees=True,return_list=False):
    """
    ref, test: (H, W, C)
    返回：所有有效像素的平均 SAM
    """
    assert ref.shape == test.shape and ref.ndim == 3

    R = ref.astype(np.float64).reshape(-1, ref.shape[-1])
    T = test.astype(np.float64).reshape(-1, test.shape[-1])

    dot = np.einsum('ij,ij->i', R, T)
    ...
    cos_theta = dot[valid] / denom[valid]
...
def ERGAS(ref, test, scale=4.0, eps=1e-15):
    """
    ERGAS 指标(ref 为参考/GT, test 为预测）
    ref, test: (H, W, C)，数据已归一化到 [0,1]
    scale: 固定为 4.0(分辨率比例)只是比例缩放, 4x和8x只影响scale, 因此固定为4.0, 8x的真实值要除以2, 论文只报道scale=4.0, 4倍和8倍也可以进行比较
    """
    assert ref.shape == test.shape and ref.ndim == 3, "ref/test 形状必须一致，且为 (H, W, C)"
    ...
    # 每个 band 的 MSE（这里等价于 RMSE^2）
    mse_c = np.mean(diff ** 2, axis=(0, 1))     # (C,)
    mu_c = np.mean(ref, axis=(0, 1))            # (C,)

    mu_c = np.maximum(mu_c, eps)                # 避免除零
    s = np.mean(mse_c / (mu_c ** 2))

    # scale 固定为 4.0
    scale = 4.0
    return 100.0 / scale * np.sqrt(s)

def PSNR(ref, test, data_range=1.0, eps=1e-15):
    """
    计算逐通道 PSNR 并求平均
    ref, test: (H, W, C)
    data_range: 信号范围, 默认为1.0, 传入的数据需要先进行归一化，即除以2047.0
    返回: mean_psnr, channel_psnr (列表)
    """
    assert ref.shape == test.shape, "ref 和 test 形状必须一致，且为 (H, W, C)"
    ...
    # 避免除零
    mse_c = np.clip(mse_c, eps, None)

    channel_psnr = 10.0 * np.log10((data_range ** 2) / mse_c)  # (C,)
    mean_psnr = channel_psnr.mean()
    return mean_psnr, channel_psnr.tolist()
